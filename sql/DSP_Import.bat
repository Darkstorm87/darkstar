...
REM "C:\Program Files\MySQL\MySQL Server 5.7\bin\mysqladmin" -h localhost -u root -pHokuten1 DROP dspdb

REM ECHO Creating Database dspdb
REM "c:\program files\mysql\mysql server 5.7\bin\mysqladmin" -h localhost -u root -pHokuten1 CREATE dspdb
...
FOR %%X IN (*.sql) DO ECHO Importing %%X & "c:\program files\mysql\mysql server 5.7\bin\mysql" dspdb -h localhost -u root -pHokuten1 < %%X
