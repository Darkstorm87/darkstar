...
int32 recv_parse(int8* buff, size_t* buffsize, sockaddr_in* from, map_session_data_t* map_session_data)
{
    ...
    if (checksumResult == 0)
    {
        ...
        {
            uint32 CharID = ref<uint32>((uint8*)buff, FFXI_HEADER_SIZE + 0x0C);
    ...
    else
    {
        ...
        // reading data size
        uint32 PacketDataSize = ref<uint32>((uint8*)buff, *buffsize - sizeof(int32) - 16);
...
int32 parse(int8* buff, size_t* buffsize, sockaddr_in* from, map_session_data_t* map_session_data)
{
    ...
    uint16 SmallPD_Code = ref<uint16>((uint8*)buff, 0);

    for (int8* SmallPD_ptr = PacketData_Begin;
        SmallPD_ptr + (ref<uint8>((uint8*)SmallPD_ptr, 1) & 0xFE) * 2 <= PacketData_End && (ref<uint8>((uint8*)SmallPD_ptr, 1) & 0xFE);
        SmallPD_ptr = SmallPD_ptr + SmallPD_Size * 2)
    {
        SmallPD_Size = (ref<uint8>((uint8*)SmallPD_ptr, 1) & 0x0FE);
        SmallPD_Type = (ref<uint16>((uint8*)SmallPD_ptr, 0) & 0x1FF);
        ...
        if (PacketSize[SmallPD_Type] == SmallPD_Size || PacketSize[SmallPD_Type] == 0) // Tests incoming packets for the correct size prior to processing
        {
            ...
            if ((ref<uint16>((uint8*)SmallPD_ptr, 2) <= map_session_data->client_packet_id) ||
                (ref<uint16>((uint8*)SmallPD_ptr, 2) > SmallPD_Code))
            {
                continue;
            }
            if (SmallPD_Type != 0x15)
            {
                ShowInfo("parse: %03hX | %04hX %04hX %02hX from user: %s\n", SmallPD_Type, ref<uint16>((uint8*)SmallPD_ptr, 2), ref<uint16>((uint8*)buff, 2), SmallPD_Size, PChar->GetName());
            }
    ...
    if (ref<uint16>((uint8*)buff, 2) != map_session_data->server_packet_id)
    {
        ref<uint16>((uint8*)map_session_data->server_packet_data, 2) = SmallPD_Code;
        ref<uint16>((uint8*)map_session_data->server_packet_data, 8) = (uint32)time(nullptr);
...
int32 send_parse(int8 *buff, size_t* buffsize, sockaddr_in* from, map_session_data_t* map_session_data)
{
    ...
    ref<uint16>((uint8*)buff, 0) = map_session_data->server_packet_id;
    ref<uint16>((uint8*)buff, 2) = map_session_data->client_packet_id;

    // сохранение текущего времени (32 BIT!)
    ref<uint32>((uint8*)buff, 8) = (uint32)time(nullptr);
    ...
    do {
        do {
            *buffsize = FFXI_HEADER_SIZE;
            ...
            ref<uint32>((uint8*)PTempBuff, zlib_compressed_size(PacketSize)) = PacketSize;
...
int32 map_config_default()
{
    ...
    map_config.mysql_login = "***REMOVED***";
    map_config.mysql_password = "***REMOVED***";
