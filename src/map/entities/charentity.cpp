...
void CCharEntity::OnDakenAttack(CBattleEntity* PTarget, action_t& action)
{
    int32 damage = 0;
    int32 totalDamage = 0;
    ...
    if (ammoThrowing && this->StatusEffectContainer->HasStatusEffect(EFFECT_SANGE))
    {
        isSange = true;
    }
    ...
    else if (dsprand::GetRandomNumber(100) < battleutils::GetRangedHitRate(this, PTarget, isBarrage)) // hit!
    {
        ...
        else
        {
            ...
            if (slot == SLOT_AMMO && PAmmo != nullptr)
            {
                charutils::TrySkillUP(this, (SKILLTYPE)PAmmo->getSkillType(), PTarget->GetMLevel());
            }
        }
