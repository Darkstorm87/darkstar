...
function onSpellCast(caster,target,spell)
    ...
    if(caster:getMerit(dsp.merit.HYOTON_SAN) ~= 0) then -- T2 mag atk/mag acc, don't want to give a penalty to entities that can cast this without merits
        bonusMab = bonusMab + caster:getMerit(dsp.merit.HYOTON_SAN) - 5 -- merit gives 5 power but no bonus with one invest, thus subtract 5
        bonusAcc = bonusAcc + caster:getMerit(dsp.merit.HYOTON_SAN) - 5
    end
